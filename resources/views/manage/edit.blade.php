<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{route('manage.'.$type.'.update', $entity->id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                                    ...
                                    <div class="grid grid-cols-3 gap-6">
                                        <div class="col-span-3 sm:col-span-2">
                                            ...
                                            <div class="mt-1 flex rounded-md shadow-sm">
                                                <input type="{{$type}}" name="{{$name}}" id="{{$name}}" value="{{$entity->$name}}"
                                            ...
                                            @if($type === 'file')
                                                <div class="mt-5">
                                                    ...
                                                    <img class=""
                                                         src="{{asset('images/'. $entity->getTable() .'/'. $entity->$name)}}"
                                                         alt="">
                                                </div>
                                            @endif
                                        </div>
                                ...
                                @elseif($type === 'checkbox')
                                    <div class="flex items-start">
                                        <div class="flex items-center h-5">
                                            ...
                                            <input id="{{$name}}" name="{{$name}}" type="{{$type}}" @if($entity->$name == 1) checked value="1" @endif class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                                        </div>
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                            @foreach($name['options'] as $key=>$value)
                                                <option @if($entity->$type == $key) selected @endif value="{{$key}}">{{$value}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                @endif
